...
class Learner(GetAttr):
    ...
    def set_opt(self):
        if self.model:
            # 設定 optimizer 優化器，
            self.opt = self.opt_func(self.model.parameters(), self.lr, weight_decay=1e-4) # 用 Adam 做優化，用指定的學習率，並在每次梯度更新時自動對權重引入 L2 正則化。
                                                                                          # 並且額外設定權重衰減（L2 正則化），weight_decay=1e-4 → 這個數值會自動讓 optimizer 在每次更新參數時，對權重施加一個額外懲罰，公式上等於對 loss 加一項 λ‖w‖²（L2 項）。← 我加的。
        else: self.opt = None
